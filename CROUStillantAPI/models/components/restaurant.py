...
class Restaurant:
    ...
    nom = openapi.String(
        ...
        example="Resto U' Cap Sud"
    )
    adresse = openapi.String(
        description="Adresse du restaurant",
        example="2 avenue Poplawski à PAU"
    )
    ...
    horaires = openapi.Array(
        ...
        example=["du lundi au vendredi", "SELF : 11h15 - 13h45 | 18h-19h30", "BAR : 11h30-14h", "SANDWICHERIE : 11h15 - 13h45", "L'OASIS (restaurant administratif) : 11h30 - 13h45"]
    )
    jours_ouvert = openapi.Array(
        items=Jours,
        description="Jours d'ouverture du restaurant",
    )
    image_url = openapi.String(
    ...
    ispmr = openapi.Boolean(
        description="Le restaurant est-il accessible aux PMR ?",
        example=True
    )
    zone = openapi.String(
        description="Zone du restaurant",
        example="Pau"
    )
